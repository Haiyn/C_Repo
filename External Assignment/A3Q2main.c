...
// NOTE: There is no inequality testing required for these four operations (remove, union, difference,
//       and intersection) due to the fact that they create a new set -- verifying that the new set is 
...
void testIntersection()
{
#define INTERSECTION_TESTS 8
  Set *setA;
  Set *setB;
  Set *intersectionSet;
  TestCase expectedItems[INTERSECTION_TESTS] = {{{}, 0, "Two empty sets", {}, 0, {}, 0}, 
                                                ...
                                                {{3, 4}, 2, "Overlapping sets", {1, 2, 3, 4}, 4, {3, 4, 5, 6}, 4}
                                               };

  printf( "\nTesting intersections:\n" );
  printf( "----------------------\n" );
  for (int i=0; i<INTERSECTION_TESTS; i++)
  {
    printf("\n%s:", expectedItems[i].testName);
    setA = buildSet(expectedItems[i].inASize, expectedItems[i].inA);
    setB = buildSet(expectedItems[i].inBSize, expectedItems[i].inB);
    ...
    if (setA && setB)
    {
      intersectionSet = intersectionOf(setA, setB);

      assert(intersectionSet);
      if (intersectionSet)
      {
        testEqual(intersectionSet, expectedItems[i].size, expectedItems[i].expected);
...
#define DIFF_TESTS 8
  ...
  TestCase expectedItems[DIFF_TESTS] = {{{}, 0, "Two empty sets", {}, 0, {}, 0}, 
                                        ...
                                        {{1}, 1, "Unique 1 item sets", {1}, 1, {4}, 1}, 
                                        {{1, 2, 3}, 3, "Unique n item sets", {1, 2, 3}, 3, {4, 5}, 2}, 
                                        {{1, 3}, 2, "Overlapping sets", {1, 2, 3, 4}, 4, {2, 4, 6}, 3}
                                       };
  ...
  for (int i=0; i<DIFF_TESTS; i++)
  {
    ...
    if (setA && setB)
    {
      diffSet = differenceOf(setA, setB);
